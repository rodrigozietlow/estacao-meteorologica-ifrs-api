...
class ChuvaModel extends Model{
	...
	public function buscarChuva(): ?Chuva {
		...
		if($temp = $stmt->fetch(PDO::FETCH_ASSOC)){
			return new Chuva($temp['valor'], $temp['data']);
		}
		else{
			return NULL;
		}
	...
	public function buscarMediaChuva(): ?Chuva {
		$stmt = $this->conexao->prepare(
			"
		...
		if($temp = $stmt->fetch(PDO::FETCH_ASSOC) and !empty($temp['data'])){
			return new Chuva($temp['media'], $temp['data']);
		}
		else{
			return NULL;
		}
	...
	public function historico($dias): Array {
		...
		while($temp = $stmt->fetch(PDO::FETCH_ASSOC)){
			$media[] = new Chuva($temp['media'], $temp['data']);
		}
