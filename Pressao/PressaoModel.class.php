...
class PressaoModel extends Model{
	...
	public function buscarPressao(): ?Pressao {
		...
		if($temp = $stmt->fetch(PDO::FETCH_ASSOC)){
			return new Pressao($temp['valor'], $temp['data']);
		}
		else{
			return NULL;
		}
	...
	public function buscarMediaPressao(): ?Pressao {
		$stmt = $this->conexao->prepare(
			"
		...
		if($temp = $stmt->fetch(PDO::FETCH_ASSOC) and !empty($temp['data'])){
			return new Pressao($temp['media'], $temp['data']);
		}
		else{
			return NULL;
		}
	...
	public function historico($dias): Array {
		...
		while($temp = $stmt->fetch(PDO::FETCH_ASSOC)){
			$media[] = new Pressao($temp['media'], $temp['data']);
		}
